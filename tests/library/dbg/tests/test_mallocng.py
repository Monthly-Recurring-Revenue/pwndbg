...
# Run the full mallocng suite against the container's system musl AND every
# per-version musl that the heap-libc-tests workflow builds + extracts. They all
# share the heap_musl.native.c source, so these assertions are version-independent
# and this is what exercises the suite across many musl versions rather than only
# the one baked into the test container. mallocng exists since musl 1.2.1.
_MUSL_DOCKERFILE = Path(__file__).resolve().parents[4] / "Dockerfile.musl-test-libs"
_MUSL_VERSIONS = re.findall(
    r"(?m)^FROM base-builder AS build-([0-9.]+)", _MUSL_DOCKERFILE.read_text()
)
...
def _mallocng_binaries() -> list[Path]:
    ...
    # Only the system binaries exist in a normal dbg run; the per-version ones are
    # present once the libc workflow has built them. `make all` fails loudly if a
    # requested binary did not build, so filtering on existence cannot hide one.
