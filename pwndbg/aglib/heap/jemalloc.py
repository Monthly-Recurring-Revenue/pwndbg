...
def _load_rtree_geom() -> _RtreeGeom:
    """Read jemalloc's actual radix-tree geometry from the target instead of
    assuming it.

    jemalloc derives the rtree's height, per-level bit splits and leaf-element
    layout from LG_VADDR -- the count of significant virtual-address bits detected
    at *build* time. That is 48 on hosts with 4-level paging but 57 on hosts with
    5-level paging (LA57; Ice Lake and newer). The two layouts differ completely::

        LG_VADDR=48 -> 2 levels, 18-bit each, 2**18 root, packed/compact leaves
        LG_VADDR=57 -> 3 levels, 15-bit each, 2**15 root, non-compact leaves

    Assuming the 48-bit layout reads wrong offsets on a binary built for 57 bits
    (out-of-bounds root indices, bogus extent pointers). That is the #3615 flake:
    each CI run rebuilds the test image, so it passed or failed purely on whether
    the build host used 4- or 5-level paging.

    jemalloc bakes its compile-time ``rtree_levels`` table and the leaf struct into
    the debuginfo, so use those as ground truth; fall back to the 48-bit layout if
    they cannot be read."""
    try:
        frame = pwndbg.dbg.selected_frame()
        ctx = frame if frame else pwndbg.dbg.selected_inferior()
